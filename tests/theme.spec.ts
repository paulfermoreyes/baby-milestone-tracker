...
test.describe("Lumina Theme Toggle UI Tests", () => {
  test("should toggle the dark/light theme classes on html tag", async ({
    page,
  }) => {
    ...
    const isDarkInitially = await htmlLocator.evaluate((el) =>
    ...
    const isDarkAfterClick = await htmlLocator.evaluate((el) =>
      el.classList.contains("dark"),
    );
    ...
    const isDarkAfterSecondClick = await htmlLocator.evaluate((el) =>
      el.classList.contains("dark"),
    );
